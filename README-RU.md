...
| `SSH_USER_OVERRIDES` | пусто | Переопределение пользователя по IP, напр. `"1.2.3.4:ubuntu 5.6.7.8:admin"` |
| `SSH_HOST_OVERRIDES` | пусто | Переопределение *адреса подключения*, напр. `"1.2.3.4:10.0.0.4"` — ходить по SSH не на тот IP, что в панели. См. [Ноды с несколькими IP](#ноды-с-несколькими-ip). |
| `SSH_PORT` | `22` | |
| `PARALLEL_JOBS` | `4` | Сколько нод обрабатывать параллельно |
| `REMOTE_RETENTION_DAYS` | `0` | Удалять **с ноды** сжатые архивы старше N суток после успешной выгрузки. `0` — ничего не удалять. См. [Хранение на нодах](#хранение-на-нодах). |
...
Коллектор выгружает архивы в S3, но по умолчанию **ничего не удаляет с ноды** — ротированные `.gz`
копятся там бесконечно. Чтобы чистить их автоматически, задайте `REMOTE_RETENTION_DAYS`:

```bash
REMOTE_RETENTION_DAYS=2
```

Удаление выполняется по каждой ноде сразу после обработки её файлов и намеренно осторожно:

- срабатывает **только если нода отработала без единой ошибки** — значит все архивы либо выгружены
  в этом прогоне, либо уже лежали в S3 (учтены как `skipped`). Одна неудачная выгрузка — и на этой
  ноде не удаляется ничего;
- удаляются **только сжатые архивы** (`*.log.*.gz`, `*.log-*.gz`). Текущий `access.log` и несжатые
  `dateext`-файлы не трогаются: коллектор пропускает их намеренно (`delaycompress`), они уедут в S3
  после следующего сжатия;
- при `REMOTE_RETENTION_DAYS=0` (по умолчанию) поведение прежнее, ничего не удаляется.

Каждая очистка пишется в лог строкой `purged N archive(s) older than Nd`.
...
- **Logrotate стандартно запускается раз в сутки** (`logrotate.timer`, `OnCalendar=daily`). С директивой `daily` (плюс `notifempty`) каждый непустой лог ротируется ежедневно независимо от размера — это даёт по одному ротированному файлу на ноду в день, так что каждая нода ежедневно вносит файлы в S3, а Telegram-отчёт читается чисто. По-настоящему мёртвые ноды (где xray совсем перестал писать, файл 0 байт) всё ещё показывают «нечего собирать» из-за `notifempty` — сигнал «нода сломана» сохраняется.

## Ноды с несколькими IP

Префикс в S3 формируется по IP ноды **как его отдаёт API панели**. Отсюда два следствия.

**У ноды сменился IP в панели.** Сборщик видит новую ноду и заново заливает всю историю с диска (до `rotate 30` дней) под новый префикс — в отчёте это 🟡 с большим числом файлов и многогигабайтным объёмом. Чтобы свести архив в одно место, копируем старый префикс в новый (server-side), проверяем, и только потом удаляем старый:

```bash
# креды S3 экспортированы из config.env
aws --endpoint-url "$S3_ENDPOINT_URL" s3 sync \
    "s3://$S3_BUCKET/${S3_PREFIX}<СТАРЫЙ_IP>/" "s3://$S3_BUCKET/${S3_PREFIX}<НОВЫЙ_IP>/" --no-progress
# сверить количество объектов и размеры, ТОЛЬКО ПОТОМ:
aws --endpoint-url "$S3_ENDPOINT_URL" s3 rm "s3://$S3_BUCKET/${S3_PREFIX}<СТАРЫЙ_IP>/" --recursive
```

Всегда `sync` → проверка → `rm`. Никогда не `mv` одним шагом.

**IP из панели недоступен или теряет пакеты со сборщика.** Если у сервера есть второй рабочий адрес — направляем на него транспорт через `SSH_HOST_OVERRIDES`, оставляя панельный IP ключом в S3:

```bash
SSH_HOST_OVERRIDES="203.0.113.10:198.51.100.10"
#                   ^ IP в панели  ^ IP для SSH
```

Подменяется только SSH/rsync-соединение — префикс в S3 остаётся по панельному IP, поэтому архив ноды не разъезжается. `./xray-logs-collector.sh test <ip-из-панели>` покажет, что подмена активна. Это обходной путь: потери на маршруте обычно означают проблему у хостера, и её стоит завести в поддержку.
